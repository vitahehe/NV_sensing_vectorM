...
def make_global_odmr_model():
    def model(x, *params):
        ...
        centers = np.array(params[:n_dips], dtype=float)
        contrasts = np.array(params[n_dips:2 * n_dips], dtype=float)

        delta_hf = float(params[2 * n_dips])
        width_L = float(params[2 * n_dips + 1])

        w_m1 = float(params[2 * n_dips + 2])
        w_0 = float(params[2 * n_dips + 3])
        w_p1 = float(params[2 * n_dips + 4])
...
def validate_frequency_units(f_hz):
    ...
    if np.max(f_hz) < 1e6:
        raise ValueError("Frequency axis looks too small to be in Hz.")

    f_ghz = f_hz / 1e9
    if not (2.0 < np.median(f_ghz) < 4.0):
        raise ValueError(
            f"Median frequency after Hz->GHz conversion is {np.median(f_ghz):.6f} GHz, which does not look like NV ODMR."
        )


def detect_8_dip_center_guesses(f_ghz, y_norm, D_guess_GHz=2.87, fmin=2.79, fmax=2.96, prominence=0.01):
    y_smooth = smooth_signal(y_norm, window=9, poly=3)
    inv = 1.0 - y_smooth

    left_mask = (f_ghz < D_guess_GHz) & (f_ghz >= fmin)
    right_mask = (f_ghz > D_guess_GHz) & (f_ghz <= fmax)
...
def fit_polynomial_baseline(f_ghz, y_raw, dip_centers, mask_halfwidth_GHz=0.008, deg=2):
    ...
    coeffs = np.polyfit(f_ghz[mask], y_raw[mask], deg=deg)
...
def fit_global_odmr(
    ...
    fmax=2.96,
    baseline_deg=2,
    baseline_mask_halfwidth_GHz=0.008,
    prominence=0.01,
    sigma=None,
):
    ...
    contrasts0 = np.array(contrasts0, dtype=float)

    delta0 = 0.00216
    widthL0 = 0.0008
    p0 = np.concatenate([centers0, contrasts0, [delta0, widthL0], [1.0, 1.0, 1.0]])
    ...
    centers = np.array(popt[:n_dips], dtype=float)
    centers_err = np.array(perr[:n_dips], dtype=float)

    contrasts = np.array(popt[n_dips:2 * n_dips], dtype=float)
    contrasts_err = np.array(perr[n_dips:2 * n_dips], dtype=float)
    ...
    w_m1 = float(popt[2 * n_dips + 2])
    w_0 = float(popt[2 * n_dips + 3])
    w_p1 = float(popt[2 * n_dips + 4])
    ...
    min_sep_MHz = np.min(np.diff(centers)) * 1000.0
    if min_sep_MHz < 0.5:
        raise RuntimeError(
            f"Fitted dip centers are too close together: minimum separation = {min_sep_MHz:.3f} MHz."
        )
    ...
    for i, (fc, fc_err, C, C_err) in enumerate(zip(centers, centers_err, contrasts, contrasts_err), start=1):
        ...
        rows.append({
            "dip_id": i,
            "dip_side": "left" if fc < D_eff else "right",
            "center_GHz": fc,
    ...
    payload = {
        ...
        "pcov": pcov,
        "perr": perr,
        "fit_success": bool(lsq.success),
        "fit_status": int(lsq.status),
        "fit_message": str(lsq.message),
    }
